...
void surface_filters::BuildSurface::onInit() {
    ...
    NODELET_DEBUG("[%s::onInit] BuildSurface Nodelet successfully created with connections:\n"
                  ...
                  " - [publisher]  markers              : %s\n",
                  getName().c_str(), getMTPrivateNodeHandle().resolveName("create_surface").c_str(),
...
void surface_filters::BuildSurface::synchronized_input_callback(const Segment::ConstPtr &segment, bool is_new) {
    ...
    //    NODELET_DEBUG(
    //            "[%s::input_callback] PointCloud with %lu data points, stamp %f, and frame %s on topic %s received.",
    ...
    auto surface_and_holes = this->publish_surface(segment, is_new);
    if (surface_and_holes.first) this->publish_surface_mesh(surface_and_holes.first, surface_and_holes.second, is_new);
}
...
auto surface_filters::BuildSurface::publish_surface(const Segment::ConstPtr &segment, bool is_new)
    ...
    std::tie(surface->surface.concave_hull, holes) =
        this->get_concave_hull(surface->surface.model, surface->surface.inliers);
...
surface_filters::BuildSurface::publish_surface_mesh(const SurfaceStamped::ConstPtr &surface, std::vector<double> &holes,
    ...
    this->get_3d_mesh(surface, holes, surface_mesh->surface_mesh.surface_mesh);
    ...
    //    NODELET_INFO_STREAM("[" << getName().c_str() << "::input_callback] " << std::setprecision(3) <<
...
auto surface_filters::BuildSurface::get_concave_hull(const pcl::ModelCoefficients &model, const PointCloudIn &cloud)
    -> std::pair<PolygonMesh, std::vector<double>> {
    ...
    for (const Triangulate::Triangle &triangle : tri.triangles()) {
        ...
        if (std::none_of(edges.begin(), edges.end(), edge_too_long_fn)) {
            ...
            // Then do remove it (and add its centroid to the holes list)
            all_holes.push_back((tri.get_x(triangle[0]) + tri.get_x(triangle[1]) + tri.get_x(triangle[2])) / 3.0);
            all_holes.push_back((tri.get_y(triangle[0]) + tri.get_y(triangle[1]) + tri.get_y(triangle[2])) / 3.0);
        }
    }
    ...
    // Remove all loops and add them as holes
    // NOTE this loop appends to hull.polygons while iterating so don't change it to use iterators or cache size()
    for (std::size_t poly_i = 0; poly_i < hull.polygons.size(); poly_i++) {
        // This algorithm assumes implicitly closed polygons, so enforce that here
        ...
        for (auto pt1_it = hull.polygons[poly_i].vertices.begin(); pt1_it != hull.polygons[poly_i].vertices.end();
            ...
            if (pt2_it != hull.polygons[poly_i].vertices.end()) {
                assert(*pt1_it == *pt2_it);

                // Add [pt1, pt2) as a new polygon (note half-closed range)
                pcl::Vertices new_polygon;
                new_polygon.vertices.insert(new_polygon.vertices.begin(), pt1_it, pt2_it);

                // Remove [pt1, pt2) from this polygon (keeps pt2 so it doesn't change the shape)
                // Also sets pt1 so the next step is valid despite invalidating iterators
                pt1_it = hull.polygons[poly_i].vertices.erase(pt1_it, pt2_it);

                // The below push_back invalidates every iterator, so be careful reorganizing
                auto pt1_idx = std::distance(hull.polygons[poly_i].vertices.begin(), pt1_it);
                hull.polygons.push_back(new_polygon);
                pt1_it = hull.polygons[poly_i].vertices.begin() + pt1_idx;
            }
        }
    }

    // Find polygons outside the outer perimeter and move them to the back of the list
    // Also removes polygons larger than a given area
    ...
    const auto inside_perimeter = [&tri, &perimeter_v](pcl::Vertices &v) -> bool { // Return true == keep
        ...
        // I have found a case where a = b++ is actually useful
        for (auto iter = v.vertices.begin(); iter != v.vertices.end(); iter_prev = iter++) {
            area_doubled += (tri.get_x(*iter) + tri.get_x(*iter_prev)) * (tri.get_y(*iter) - tri.get_y(*iter_prev));
        }

        if (area_doubled < 0) { // Then the polygon was the wrong way round
            std::cout << "Reversing a polygon with " << v.vertices.size() << " points" << std::endl;
    ...
    const auto remove_start = std::partition(hull.polygons.begin() + 1, hull.polygons.end(), inside_perimeter);
    ...
    // Mimics std::remove_copy_if but acts pairwise
    std::vector<double> holes;
    for (auto x_it = all_holes.begin(); x_it != all_holes.end(); std::advance(x_it, 2)) {
        auto y_it = std::next(x_it);

        // If the hole point is inside any of the hole polygons, then keep it
        if (std::any_of(hull.polygons.begin() + 1, hull.polygons.end(), [=, &tri](const pcl::Vertices &v) {
                return tri.point_in_polygon(*x_it, *y_it, v.vertices.begin(), v.vertices.end());
            })) {
            holes.push_back(*x_it);
            holes.push_back(*y_it);
        }
    }
...
void surface_filters::BuildSurface::get_projected_cloud(const Segment::ConstPtr &segment, PointCloudIn &proj_output) {
...
void surface_filters::BuildSurface::get_3d_mesh(const SurfaceStamped::ConstPtr &surface, std::vector<double> &holes,
    ...
    this->get_triangulation_segments(surface->surface.concave_hull, points_2d, segment_list);
    assert(points_2d.size() == hull_cloud.size() * 2);
...
void surface_filters::BuildSurface::get_triangulation_segments(const PolygonMesh &hull,
                                                               ...
                                                               std::vector<int> &segment_list) const {
...
void surface_filters::BuildSurface::get_triangluation_trimesh(std::vector<double> &points_2d,
    ...
    if (num_points_out > num_points_in) {
        ...
        std::transform(
            additional_x.begin(), additional_x.end(), additional_y.begin(),
            ...
            [&existing_points](const double &additional_pt_x, const double &additional_pt_y) -> long {
                PointIn overflow_pt(static_cast<float>(additional_pt_x), static_cast<float>(additional_pt_y), 0);
        ...
        if (num_points_out > num_points_in) {
            for (std::size_t idx = 0; idx < output_trimesh.triangles[i].vertex_indices.size(); idx++) {
                ...
                if (old_val >= num_points_in) {
                    ...
                    //                    NODELET_DEBUG_STREAM("Triangulate at index " << i << " coordinate " << idx <<
                    //                    " was out of bounds (" <<
                    //                                         old_val << "), replaced with closest point: " <<
                    //                                         new_val);
                }
            }
        }
            ...
            for (auto pt2 = pt1 + 1; pt2 < hull_cloud.end(); ++pt2) {
                if (pcl::squaredEuclideanDistance(*pt1, *pt2) < 1e-10) {
                    NODELET_ERROR_STREAM("Hull invalid -- point " << std::distance(hull_cloud.begin(), pt1) << " is at "
                                                                  << " virtually the same location as "
                                                                  << std::distance(hull_cloud.begin(), pt2));
                    return false;
                }
            }
        }
...
bool surface_filters::BuildSurface::verify_hull_no_self_intersection(const PolygonMesh &hull,
    ...
    //
    //        // It is acceptable, but not required, for the front to be equal to the back (explicitly closed polygon)
    //        // If the last element is equal to the first, ignore it when iterating and treat polygon as implicitly
    //        closed
    ...
    //
    //            // Make a range that skips the previous segment unless this is the first segment of the current
    //            polygon
    ...
    ////                    NODELET_WARN_STREAM("Segments share an endpoint and won't be checked for
    /// self-intersection");
    //                } else if (geeksforgeeks::doIntersect(segment.first, segment.second, next_seg.first,
    //                next_seg.second)) {
    ...
    for (const pcl::Vertices &vertices : hull.polygons) {
        ...
        for (std::size_t i = 0; i + 1 < last_index; i++) {
            ...
            if (segment_length > alpha_ * 2) {
                NODELET_ERROR_STREAM("Hull invalid -- there is a segment with length greater than 2 * "
                                     << alpha_ << " (length is " << segment_length << ")");
                return false;
            }
        }
    }
