...
private:
    ...
    std::pair<SurfaceStamped::Ptr, std::vector<double>> publish_surface(const Segment::ConstPtr &segment, bool is_new);

    SurfaceMeshStamped::Ptr publish_surface_mesh(const SurfaceStamped::ConstPtr &surface, std::vector<double> &holes,
                                                 bool is_new);

    std::pair<PolygonMesh, std::vector<double>> get_concave_hull(const pcl::ModelCoefficients &model,
                                                                 const PointCloudIn &cloud);

    void get_projected_cloud(const Segment::ConstPtr &segment, PointCloudIn &proj_output);

    void get_3d_mesh(const SurfaceStamped::ConstPtr &surface, std::vector<double> &holes,
