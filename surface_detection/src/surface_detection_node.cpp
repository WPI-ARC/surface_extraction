...
public:
    ...
    bool go(SurfaceDetectionRequest &req, SurfaceDetectionResponse &resp) {
        ...
        resp = surface_detection.detect_surfaces_within(center, extents, vis);
...
int main(int argc, char **argv) {
    ...
    // Make publishers
    auto pp_pub = n.advertise<SurfaceDetection::PointCloud>(pn.param("unprocessed_points_topic", std::string("/pending_points")), 1);

    // Make subscribers
    auto sub = n.subscribe(pn.param("input_points_topic", std::string("/scan_cloud")), 100, &SurfaceDetection::add_points,
                           &surface_detection);

    // Make timers
    auto pp_timer = n.createTimer(ros::Duration(1), [&surface_detection, &pp_pub, &target_frame](const ros::TimerEvent &) {
    ...
    if (pn.getParam("wait_for_points", n_points_to_wait_for)) {
        ros::Rate r(2);
        while (ros::ok() && surface_detection.get_num_pending_points() < static_cast<std::size_t>(n_points_to_wait_for)) {
            ROS_INFO_STREAM_DELAYED_THROTTLE(2, "Waiting for " << n_points_to_wait_for << " points before advertising service");
            r.sleep();
            ros::spinOnce();
        }
    } else {
